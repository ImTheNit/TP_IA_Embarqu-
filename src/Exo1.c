...
int writeCSV(char * file, int valueA, int valueB){
...
int getError(int classe, int reponse){
    return classe - reponse;
}

void apprendre(double weight[ROWS*COLLUMN], double neurones[ROWS*COLLUMN], int error){
    int i =0;
    for (i; i< ROWS*COLLUMN; i ++){
...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        error = getError(classe, reponseNeurone);
        #ifdef DEBUG
        ...
        errors[0] = getError(0,reponsesNeurone[0]);
        errors[1] = getError(1,reponsesNeurone[1]);
        ...
        /*  */
        totalError = abs(errors[0]) + abs(errors[1]);
        #ifdef ERROR
        fprintf(stdout,"Erreurs : %d %d\n",errors[0],errors[1]);
        fprintf(stdout,"totalError : %d\n\n",totalError);
        #endif
    ...
    for (int i = 0; i < 2; i++){ 
        ...
        for (int j = 0; j < 25; j++){
            ...
            for (int k = 0 ; k < 50; k++){
                ...
                errorTest = getError(expectedClasse, reponseNeuroneTest);

                if (errorTest != 0 ){   //Pas la bonne réponse
                    nbWrongByNoise++;
